...
The shortcut files are in the [`output/`](../output/) folder of this repository. Install all three on your iPhone:
...
**From your iPhone (easiest):**
1. Open this repository in Safari on your iPhone
2. Navigate to the `output/` folder
3. Tap each `.shortcut` file and tap "Add Shortcut" when prompted

**From your computer:**
- **AirDrop (recommended):** Right-click each `.shortcut` file > Share > AirDrop > select your iPhone
- **iCloud Drive:** Copy the three files to iCloud Drive, then tap each one in the Files app on your iPhone
- **Email:** Attach the three files to an email, open on iPhone, tap each to install
...
- **KM** is the odometer reading (or `-` if tracking is disabled)
- **Purpose** is `Business`, `Private`, or `-` if classification is disabled
...
If you want to customize the shortcut behavior, you can regenerate the `.shortcut` files:
...
Edit `src/shortcut_generator/shortcuts.py` to change what the shortcuts do (e.g., change the log format, add extra prompts).
...
- If you are in an area with poor GPS, the city lookup may be inaccurate
...
**I have two cars.**
- Create separate Bluetooth automations for each car's device. Both automations can call the same Trip Start / Trip End shortcuts.

**I want to change my preferences.**
- Re-run the **Travel Tracker Setup** shortcut at any time to reconfigure.
